...
fun MovieDetailScreen(
    ...
    var detailMovies by remember { mutableStateOf<MovieDto?>(null) }

    val apiService = RetrofitClient.retrofitInstance.create(ApiService::class.java)

    apiService.getMoviesById(movieId).enqueue(object : Callback<MovieDto>{
        override fun onResponse(
            call: Call<MovieDto?>,
            response: Response<MovieDto?>
        ) {
            if (response.isSuccessful){
                detailMovies = response.body()
            } else{
...
@Composable
fun MovieDetailContent(movie: MovieDto){
    Column(
        modifier = Modifier.fillMaxSize()
    ) {
        AsyncImage(
